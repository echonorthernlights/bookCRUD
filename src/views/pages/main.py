...
from src.controllers.book_controller import add_new_book, get_all_books, get_single_book,update_single_book, delete_single_book
...
create_label(frame, text="Book ID : ", row=2, column=0)
...
id_entry = create_entry(frame, row=2, column=1)

data_list = create_listbox(frame, {"row": 3, "column": 0},
                           {"height": 6, "width": 65},
...
create_button(frame, cmd=lambda: update_single_book(title_entry, author_entry, year_entry, isbn_entry, id_entry), text="Update One", position={"row": 5, "column": 3})
create_button(frame, cmd=lambda : delete_single_book(id_entry), text="Delete One", position={"row": 6, "column": 3})
# create_button(frame, cmd=None, text="Delete Selected", position={"row": 7, "column": 3})
create_button(frame, text="Quit", cmd=lambda :root.destroy(), position={"row": 7, "column": 3})
